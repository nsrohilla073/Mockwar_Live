...
function App() {
 ...
 const fetchGlobalLeaderboard = async (tableId) => {
  ...
  const openLeaderboard = () => {
    ...
    if (liveTables.length > 0) {
      // Puraana: fetchGlobalLeaderboard(liveTables[0].slug);
      fetchGlobalLeaderboard(liveTables[0].id); // 🔥 Naya: ID bhejein
    }
  };
          ...
          <div className="bg-slate-900 border border-slate-800 p-6 rounded-[2rem] max-w-sm w-full shadow-2xl relative overflow-hidden">
            ...
            <div className="flex gap-2 mb-4 overflow-x-auto pb-2 scrollbar-hide relative z-10">
              {liveTables.map(t => (
                 <button 
                   key={t.id} 
                   onClick={() => fetchGlobalLeaderboard(t.id)} 
                   className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap transition-all shadow-md ${leaderboardCategory === t.id ? "bg-gradient-to-r from-yellow-500 to-amber-500 text-slate-900 shadow-[0_0_15px_rgba(245,158,11,0.4)]" : "bg-slate-800 text-slate-400 hover:bg-slate-700"}`}
                 >
                    {t.category_name}
                 </button>
              ))}
