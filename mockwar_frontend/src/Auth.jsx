...
import { Smartphone, Mail, ShieldCheck, User, MapPin, CalendarDays, Zap, Camera, Check, Gift, CheckSquare, Square } from 'lucide-react'; // 🔴 NAYA: Icons added
const API_BASE = "https://mockwar-backend.onrender.com";

// 🔴 NAYA: Legal Validations Data
...
function Auth() {
  ...
  const startCamera = async () => { /* Same as before */
    ...
    } catch (err) { alert("Camera access denied! Live photo is mandatory for KYC."); }
  };

  const capturePhoto = () => { /* Same as before */
  ...
  useEffect(() => {
    if (isCameraOpen && videoRef.current && mediaStream) { videoRef.current.srcObject = mediaStream; }
  ...
  const handleGoogleLogin = async () => { /* Same as before */
    ...
    } catch (error) { alert("Google Login Failed!"); setLoading(false); }
  };
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      setLoading(false);
    } catch (error) { alert("Failed to send OTP."); setLoading(false); }
  };
  ...
  const verifyOTP = async () => { /* Same as before */
    ...
    try {
      ...
      await verifyTokenWithDjango(token);
    } catch (error) { alert("Incorrect OTP!"); setLoading(false); }
  };
  ...
  const verifyTokenWithDjango = async (idToken) => { /* Same as before */
    ...
    } catch (error) { alert("Server error during verification."); setLoading(false); }
  };

  const handleRegistrationSubmit = async (e) => {
    e.preventDefault();
    
    // 🔴 NAYA: Legal Validations Web
    if (!isAgeVerified) return alert("You must confirm you are 18+ years old.");
    if (BANNED_STATES.includes(regData.state)) return alert("Real-money gaming is legally restricted in your state.");
          ...
          <form onSubmit={handleRegistrationSubmit} className="space-y-4 animate-fade-in">
            ...
            {regData.referred_by && (
              <div className="bg-amber-500/10 border border-amber-500/20 p-2.5 rounded-xl mb-4 flex items-center justify-center gap-2">
                ...
                <p className="text-[10px] text-amber-400 font-black uppercase tracking-widest">Referral Applied: {regData.referred_by}</p>
              </div>
            )}
            ...
            {/* 🔴 NAYA: 18+ Checkbox in Web */}
            <div className="flex items-center gap-3 mt-4 cursor-pointer" onClick={() => setIsAgeVerified(!isAgeVerified)}>
              {isAgeVerified ? <CheckSquare size={20} className="text-emerald-500" /> : <Square size={20} className="text-slate-500" />}
              <span className="text-[10px] text-slate-400 font-bold leading-tight">I certify that I am 18+ years old and not playing from restricted states.</span>
            </div>
              ...
              <div className="space-y-4 animate-fade-in">
                {!otpSent ? (
                  <>
                    ...
                    {/* 🔴 NAYA: OTP Timer UI Web */}
                    <button onClick={sendOTP} disabled={loading || timer > 0} className={`w-full text-white font-black text-sm uppercase tracking-widest py-4 rounded-xl transition-all active:scale-95 disabled:opacity-50 ${timer > 0 ? 'bg-slate-700' : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 shadow-[0_0_20px_rgba(59,130,246,0.3)]'}`}>
                      {loading ? "CONNECTING..." : (timer > 0 ? `RESEND IN ${timer}S` : "SECURE OTP LOGIN")}
                    </button>
                  </>
                ) : (
                  <>
                    <div className="flex justify-between items-end mb-2 px-1">
                      ...
                      <button onClick={() => { setOtpSent(false); setTimer(0); }} className="text-[10px] text-blue-400 font-black uppercase tracking-widest hover:text-blue-300 underline">
                        Edit Number
                      </button>
                    </div>
